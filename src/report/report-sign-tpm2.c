...
static int build_ordinary_key_data(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to serialize parent handle");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to marshal private area for new ordinary signing key");

        _cleanup_(sd_json_variant_unrefp) sd_json_variant *v = NULL;
        r = sd_json_buildo(
                        &v,
                        SD_JSON_BUILD_PAIR_STRING("type", signing_key_type_to_string(SIGNING_KEY_ORDINARY)),
                        JSON_BUILD_PAIR_IOVEC_HEX("parentHandle", &parent_handle_buf),
...
static int load_key_context(Tpm2Context *c, int dir_fd, const char *name, Tpm2Handle **ret) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to unmarshal saved signing key context '%s'", name);

        _cleanup_(tpm2_handle_freep) Tpm2Handle *handle = NULL;
        r = tpm2_load_saved_handle_context(c, &context, /* ret_name= */ NULL, &handle);
        if (r < 0)
                return log_error_errno(r, "Failed to load saved signing key context '%s'", name);
...
static int save_key_context(Tpm2Context *c, int dir_fd, const char *name, const Tpm2Handle *handle) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to save signing key context '%s'", name);

        _cleanup_free_ void *buf = NULL;
        size_t buf_sz;
        r = tpm2_marshal_saved_handle_context(context, &buf, &buf_sz);
        if (r < 0)
                return log_error_errno(r, "Failed to marshal signing key context '%s'", name);
...
static int build_report(sd_json_variant **v, const Tpm2Report *report, const struct iovec *voucher) {
        ...
        if (r < 0)
                return log_error_errno(r, "Cannot convert public key to JSON");

        _cleanup_free_ char *pubkey_pem = NULL;
        r = tpm2_tpmt_public_to_pem(report->public_key, &pubkey_pem);
        if (r < 0)
                return log_error_errno(r, "Cannot convert public key to PEM");
        ...
        FOREACH_ARRAY(c, report->components, report->n_components) {
                ...
                if (c->type == TPM2_REPORT_TYPE_NVPCR) {
                        ...
                        if (r < 0)
                                return log_error_errno(r, "Cannot convert NvPCR public area to JSON");
                }
...
static int vl_method_sign(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to load any signing keys.");
...
static int build_primary_key_data(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to marshal template for new primary signing key");
...
static int persist_and_build_key_data(
        ...
        if (r < 0) {
                ...
                return log_error_errno(r, "Failed to serialize persistent signing key handle");
        }
        ...
        r = sd_json_buildo(
                        ...
                        JSON_BUILD_PAIR_IOVEC_HEX("handle", &handle_buf));
        if (r < 0) {
                (void) tpm2_evict_handle(c, /* session= */ NULL, persistent_handle_index);
                return log_error_errno(r, "Failed to build signing key data");
        }
...
static int vl_method_create_key(
        ...
        if (as_primary) {
                ...
                * or "parentContext" rather than a "hierarchy". */
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to create new signing key in TPM");
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to convert public area to JSON");

        _cleanup_free_ char *public_pem = NULL;
        r = tpm2_tpmt_public_to_pem(&public->publicArea, &public_pem);
        if (r < 0)
                return log_error_errno(r, "Failed to convert public area to PEM");
        ...
        if (data->type == SIGNING_KEY_PERSISTENT) {
                ...
                if (r < 0)
                        ...
                        if (r < 0)
                                ...
                                return log_error_errno(r, "Failed to determine persistent handle for signing key '%s', not evicting it.", p.name);
...
static int list_signing_key(
        ...
        if (available) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to read public area of signing key '%s'", name);
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to unmarshal public area of signing key '%s'", name);
        ...
        if (have_public) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to convert public area of signing key '%s' to JSON", name);

                r = tpm2_tpmt_public_to_pem(&public_area, &public_pem);
                if (r < 0)
                        return log_error_errno(r, "Failed to convert public area of signing key '%s' to PEM", name);
        }
        ...
        if (data->type == SIGNING_KEY_PERSISTENT) {
                ...
                if (r < 0)
                        return log_error_errno(r, "Failed to deserialize persistent handle of signing key '%s'", name);

                /* This requires a new enough tss2. */
                TPM2_HANDLE index = 0;
